//Lost 10 points in the coding section because program did not produce correct answer for dataset 2.
#include <wb.h>
...
int main(int argc, char **argv) {
  ...
  //@@ Initialize the grid and block dimensions here
    //dim3 dimensionBlock(2, 2, 1);
    //dim3 dimensionGrid(ceil((numCColumns * 1.0) / 2), ceil((numCRows * 1.0) / 2), 1);
    dim3 dimensionBlock(32, 32, 1);
    dim3 dimensionGrid((numCRows - 1)/16, (numCColumns-1)/16, 1);

  wbTime_start(Compute, "Performing CUDA computation");
  //@@ Launch the GPU Kernel here
  matrixMultiply<<<dimensionGrid, dimensionBlock>>>(deviceA, deviceB, deviceC, numARows, numAColumns, numBRows,numBColumns, numCRows, numCColumns);
